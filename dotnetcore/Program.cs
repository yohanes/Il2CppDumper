...
namespace Il2CppDumper
{
    class Program
    {
        ...
        private static Config config = new Config();        private static Dictionary<Il2CppMethodDefinition, string> methodModifiers = new Dictionary<Il2CppMethodDefinition, string>();
        private static Dictionary<Il2CppTypeDefinition, int> typeDefImageIndices = new Dictionary<Il2CppTypeDefinition, int>();

        private static string UnityVersion;
        private static string DumpMethod;

        static void ShowHelp(string programName)
        {
            Console.WriteLine($"usage: {programName} path/to/global-metadata.dat path/to/libil2cpp.so unityversion dumpmethod(1-4)");
            Environment.Exit(0);
        }
        ...
        private static bool Init(byte[] il2cppBytes, byte[] metadataBytes)
        {
            ...
            {
                var stringVersion = UnityVersion;
            ...
            if (DumpMethod == null || DumpMethod.Length==0) {
                Console.WriteLine("Select Mode: 1.Manual 2.Auto 3.Auto(Plus) 4.Auto(Symbol)");
                modeKey = Console.ReadKey(true);
            } else {
                modeKey = new ConsoleKeyInfo(DumpMethod[0], ConsoleKey.A /*dummy*/, false, false, false);
            }
        ...
        private static void Dump()
        {
            ...
            if (config.DummyDll)
            {
                if (!File.Exists("Il2CppDummyDll.dll.template")) 
                {
                    Console.WriteLine("Can not find Dummy DLL template Il2CppDummyDll.dll.template. Not generating DLL. ");
                    return;
                }
                byte[] bytes = File.ReadAllBytes("Il2CppDummyDll.dll.template");
                ...
                File.WriteAllBytes("Il2CppDummyDll.dll", bytes);
